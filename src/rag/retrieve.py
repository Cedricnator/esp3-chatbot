...
class FaissRetriever:
	def __init__(self, indx_path: str, chunks_path: str, mapping_path: str, logger: LoggerAdapter) -> None:
		...
		self.index = self.load_faiss_index(self.index_path)  # type: ignore
	...
	def load_chunks_and_mapping(self, chunks_path: str, mapping_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
		chunks = pd.read_parquet(chunks_path) # type: ignore
		mapping = pd.read_parquet(mapping_path) # type: ignore
		return chunks, mapping

	def load_faiss_index(self, index_path: str) -> faiss.Index:
		idx = faiss.read_index(str(index_path)) # type: ignore
		return idx # type: ignore

	def _build_position_lookup(self) -> Dict[int, str]:
		if {"position", "chunk_id"}.issubset(set(self.mapping.columns)):
			lookup: Dict[int, str] = {}
			for _, record in self.mapping.iterrows():  # type: ignore[call-arg]
				try:
	...
	def _embed_query(self, query: str) -> Any:
		emb_raw = self._model.encode(  # type: ignore[call-overload]
	...
	def _chunk_from_position(self, position: int) -> dict[str, Any]:
		if 0 <= position < len(self.chunks):
			return self.chunks.iloc[position].to_dict()  # type: ignore
	...
	def search(self, query: str, top_k: int = 10) -> pd.DataFrame:
		...
		scores = cast(List[float], distances[0].tolist())  # type: ignore
		positions = [int(x) for x in cast(List[int], indices[0].tolist())]  # type: ignore
		rows = []
		for score, pos in zip(scores, positions):  # type: ignore[arg-type]
			if pos < 0:
				continue
			try:
